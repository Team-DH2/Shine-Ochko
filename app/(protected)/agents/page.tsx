...
import AgentCard from "@/components/us/Agent";
...
const Agents = ({
  ...
  useEffect(() => {
    fetch("/api/agents")
      .then((res) => res.json())
      .then((data) => {
        const sorted = data.sort((a: AgentDB, b: AgentDB) => b.rating - a.rating);
        setAgents(sorted);
  ...
  const handleFilter = () => {
    ...
    if (availability !== "all") {
      result = result.filter((a) => a.status === availability);
    }

    result = result.filter(
      (a) => a.rating >= Number(minRating) && a.rating <= Number(maxRating)
    );
    ...
    <div className="min-h-screen bg-black text-white px-4 sm:px-6 lg:px-12 xl:px-24">
      ...
      <div className="pt-20 pb-10 sm:pt-24 sm:pb-12 text-center">
        ...
        <p className="mt-3 text-zinc-400 text-sm sm:text-base max-w-xl mx-auto">
          Манай мэргэжлийн агентуудаас сонгон захиалаарай
        </p>
      </div>
      ...
      <div className="py-12">
        {filteredAgents.length > 0 ? (
          ...
          <div className="text-center py-20">
            <p className="text-zinc-500 text-lg">Агент олдсонгүй</p>
          </div>
        )}
      </div>
    </div>
